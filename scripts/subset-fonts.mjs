...
const TEXT = LATIN_BASIC + LATIN_1_SUPPLEMENT + LATIN_EXTENDED_A + GENERAL_PUNCTUATION + CURRENCY;
...
for (const { input, output } of targets) {
  ...
  console.log(`${input} -> ${output}: ${buf.length} -> ${subset.length} bytes (${ratio}% smaller)`);
  await rm(input);
}
