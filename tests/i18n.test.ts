...
const collectKeyPaths = (value: unknown, prefix = ""): string[] => {
    ...
    return Object.entries(value as Record<string, unknown>).flatMap(
        ([k, v]) => collectKeyPaths(v, prefix ? `${prefix}.${k}` : k),
    );
};
...
describe("i18n helpers", () => {
    ...
    it("every locale dictionary has the same array lengths for content collections", () => {
        ...
        for (const loc of locales) {
            ...
            expect(dict.resume.education.length).toBe(
                refDict.resume.education.length,
            );
            expect(dict.resume.experience.length).toBe(
                refDict.resume.experience.length,
            );
    ...
    it("localeLabels has a flag and code for every locale", () => {
        for (const loc of locales) {
            expect(localeLabels[loc].flag).toMatch(
                /\p{Regional_Indicator}\p{Regional_Indicator}/u,
            );
